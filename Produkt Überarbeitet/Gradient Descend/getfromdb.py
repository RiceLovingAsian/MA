...
#DBboi1 = os.path.join(dirname, 'RICARDO.db')
#DBboi2 = os.path.join(dirname, './Autoscout/AUTOSCOUT.db')
DBboi1 = os.path.realpath('RICARDO.db')
...
#CSV Datei wird gelesen und in dataframe konvertiert
data = pd.read_csv('RICARDO.csv')

#dictionary von allen new erzeugten numerischen Listen
d = {'brand_new':brandenumerated,'getriebsart_new':getriebsartenumerated,'antriebsart_new':antriebsartenumerated,
     'treibstoff_new':treibstoffenumerated,'verbrauch_new':verbrauchenumerated,'leergewicht_new':leergewichtenumerated,
     'hubraum_new':hubraumenumerated}

#Dictionary wird in dataframe konvertiert
df_data_numerical = pd.DataFrame(d)

#Verkettung von alter und neuer Datenframe
df_result = pd.concat([data, df_data_numerical], axis=1, join='inner')

df_result.drop(['getriebsart', 'brand','antriebsart','treibstoff','verbrauch','leergewicht','hubraum','modell'], axis=1, inplace=True)
